...
export function createOrLoadProfile(username) {
  ...
  const profile = touchProfile(existing || {
    username: clean,
    totalAnswered: 0,
    totalCorrect: 0,
    wrongQuestionIds: [],
...
export function setGuestMode(value) {
  ...
  if (value) data.activeUsername = null;
  safeWrite(data);
}
...
function updateProfile(mutator) {
  ...
  if (!username || !data.profiles[username]) return null;
  const nextProfile = touchProfile(mutator({ ...data.profiles[username] }));
